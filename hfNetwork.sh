...
function generarChannelArtifacts(){
    ...
    fabric-samples/bin/configtxgen -profile LaptopsOrdererGenesis -channelID syschain  -outputBlock ./channel-artifacts/genesis.block
    
    echo
	echo "#################################################################"
	echo "#######    Generating anchor peer update for MSP   ##########"
	echo "#################################################################"
    fabric-samples/bin/configtxgen -profile laptopschannel -outputCreateChannelTx ./channel-artifacts/laptopschannel.tx -channelID "laptopschannel"

    echo
	echo "#################################################################"
	echo "#####   Generating anchor peer update for microsoftMSP   ########"
	echo "#################################################################"
	fabric-samples/bin/configtxgen -profile laptopschannel -outputAnchorPeersUpdate ./channel-artifacts/microsoftMSPanchors.tx -channelID "laptopschannel" -asOrg microsoftMSP

	echo
	echo "#################################################################"
	echo "#######   Generating anchor peer update for appleMSP   ##########"
	echo "#################################################################"
	fabric-samples/bin/configtxgen -profile laptopschannel -outputAnchorPeersUpdate ./channel-artifacts/appleMSPanchors.tx -channelID "laptopschannel" -asOrg appleMSP
	echo
}
